...
Aktuelles Release: **v0.5.4-test44** (prerelease auf GitHub)
...
| test44 | Fixflow-Politur: rote „Code ungültig"-Meldung erschien schon beim Öffnen (HA ruft Init-Schritt mit `{}` auf). Guard von `is not None` auf Truthiness (`if user_input:`) — Fehler nur noch nach echter Eingabe. | `repairs.py` | ⏳ Deploy läuft |
