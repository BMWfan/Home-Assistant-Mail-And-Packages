...
| **test56** | **Section-Reihenfolge in Schritt 2 final:** Sendungsstatus-Quelle → Amazon → DHL → Mail-Ordner → Bild-Optionen. Manifest 0.5.9. | `config_flow.py`, `manifest.json` | ✅ **live vom User bestätigt** („Passt") |
| test55 | Die drei Section-Boxen **über** den Mail-Ordner verschoben (Schema-Reihenfolge: Sections vor `CONF_FOLDER`; HA rendert in Schema-Reihenfolge). Manifest 0.5.8. | `config_flow.py`, `manifest.json` | ✅ lädt sauber |
| test54 | 17track-No-Key-Hinweis präzisiert: „abbrechen mit ✕ → **Assistent neu starten** → in Schritt 2 „Mail-Tracking"" (vorher suggerierte es Inline-Auswahl). Manifest 0.5.7. | `strings.json`, `translations/{en,de}.json`, `manifest.json` | ✅ lädt sauber |
| test53 | 17track-No-Key-Hinweis: „geh zurück" entfernt (kein Zurück-Button im separaten Schritt) → „mit ✕ abbrechen". Manifest 0.5.6. | `strings.json`, `translations/{en,de}.json`, `manifest.json` | ✅ lädt sauber |
